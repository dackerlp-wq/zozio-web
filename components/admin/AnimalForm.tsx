...
export function AnimalForm({
  ...
  const [form, setForm] = useState({
    ...
    status:           animal?.status           ?? 'intake',          // rescue
  ...
  const handleSubmit = async () => {
    ...
    // Detekuj změnu stavu pro historii
    ...
    const payload = isShelter ? {
      ...
      medications: form.medications || null, medical_notes: form.medical_notes || null,
      in_quarantine: form.in_quarantine, quarantine_until: form.quarantine_until || null,
      quarantine_reason: form.quarantine_reason || null,
      in_foster: form.in_foster, foster_name: form.foster_name || null,
      foster_phone: form.foster_phone || null, foster_since: form.foster_since || null,
      intake_reason: form.intake_reason || null, intake_date: form.intake_date || null,
      intake_notes: form.intake_notes || null,
      found_location: form.found_location || null, found_date: form.found_date || null,
      finder_name: form.finder_name || null, finder_phone: form.finder_phone || null,
      previous_owner: form.previous_owner || null, previous_owner_phone: form.previous_owner_phone || null,
      internal_notes: form.internal_notes || null, staff_assigned: form.staff_assigned || null,
      ...
      status: form.status,
      health_status: form.health_status,
      cause_of_injury: form.cause_of_injury || null, diagnosis: form.diagnosis || null,
      treatment_notes: form.treatment_notes || null, medical_notes: form.medical_notes || null,
      public_description: form.public_description || null,
      found_location: form.found_location || null, found_date: form.found_date || null,
      finder_name: form.finder_name || null, finder_phone: form.finder_phone || null,
      vet_name: form.vet_name || null, vet_phone: form.vet_phone || null,
      last_vet_visit: form.last_vet_visit || null, medications: form.medications || null,
      in_quarantine: form.in_quarantine, quarantine_until: form.quarantine_until || null,
      weight_g: form.weight_g ? parseInt(form.weight_g) : null,
      ring_number: form.ring_number || null, release_location: form.release_location || null,
      internal_notes: form.internal_notes || null, staff_assigned: form.staff_assigned || null,
      photos, primary_photo: primaryPhoto || null, published: form.published,
    }
    ...
    // Zaznamenej změnu stavu
    if (statusChanged && data.success) {
      await fetch('/api/animal-status-history', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          animal_id:      isShelter ? animal.id : null,
          rescue_case_id: !isShelter ? animal.id : null,
  ...
  const tabs: { key: Tab; label: string; icon: string }[] = [
    ...
    ...(mode === 'edit' ? [{ key: 'history' as Tab, label: 'Historie', icon: '📋' }] : []),
  ]

  return (
    <div className="flex flex-col lg:grid lg:grid-cols-3 gap-5">

      {/* Levý panel — taby + obsah */}
      <div className="lg:col-span-2 space-y-4">
          ...
          {tabs.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`flex items-center gap-1.5 px-3 py-2 rounded-md font-display font-bold text-sm transition-all cursor-pointer border-none
                ${tab === t.key ? 'bg-espresso text-white' : 'text-gray hover:bg-sand text-espresso bg-transparent'}`}>
              <span>{t.icon}</span>
            ...
            {isShelter ? (
              <>
                ...
                <Field label="Popis pro adoptivní rodiny">
                  <textarea value={form.description} onChange={e => update('description', e.target.value)} rows={4} className={`${inputCls} resize-none`} />
                </Field>
              ...
              <>
                <Field label="Odhadovaný věk">
                  <input value={form.estimated_age} onChange={e => update('estimated_age', e.target.value)} placeholder="Dospělý, Mládě..." className={inputCls} />
                </Field>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Field label="Váha (g)">
                ...
                <Field label="Veřejný popis (pro sbírky a web)">
          ...
          <div className="bg-white rounded-lg p-5 md:p-6 border border-gray-pale shadow-sm space-y-4">
            ...
            {isShelter && (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {[
                  ...
                  <label key={key} className="flex items-center gap-3 cursor-pointer bg-sand rounded-md p-3">
                    <input type="checkbox" checked={(form as any)[key]} onChange={e => update(key, e.target.checked)} className={checkCls} />
                    <span className="text-sm font-semibold text-espresso">{label}</span>
                  </label>
                ))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Field label="Telefon na veterináře">
                <input value={form.vet_phone} onChange={e => update('vet_phone', e.target.value)} placeholder="+420 777 123 456" className={inputCls} />
              </Field>
            </div>
            ...
            <Field label="Zdravotní poznámky">
              <textarea value={form.medical_notes} onChange={e => update('medical_notes', e.target.value)} rows={4}
                placeholder="Podrobné záznamy o zdravotním stavu..." className={`${inputCls} resize-none`} />
            </Field>
            ...
            <div className="pt-4 border-t border-gray-pale">
              ...
              {form.in_quarantine && (
                <div className="space-y-3 bg-amber-light/50 rounded-md p-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {isShelter && (
              <div className="pt-4 border-t border-gray-pale">
                ...
                {form.in_foster && (
                  <div className="space-y-3 bg-rescue-bg/50 rounded-md p-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <Field label="Foster jméno">
                        <input value={form.foster_name} onChange={e => update('foster_name', e.target.value)} className={inputCls} />
                      </Field>
                      <Field label="Foster telefon">
        ...
        {tab === 'origin' && (
          <div className="bg-white rounded-lg p-5 md:p-6 border border-gray-pale shadow-sm space-y-4">
            ...
            <Field label="Poznámky k příjmu">
              <textarea value={form.intake_notes} onChange={e => update('intake_notes', e.target.value)} rows={3}
                placeholder="Okolnosti příjmu, stav při příjmu..." className={`${inputCls} resize-none`} />
            </Field>
        ...
        {tab === 'internal' && (
          <div className="bg-white rounded-lg p-5 md:p-6 border border-gray-pale shadow-sm space-y-4">
            <h2 className="font-display font-extrabold text-xl text-espresso">
              🔒 Interní záznamy
            </h2>
            <p className="text-xs text-gray">Tyto informace se nezobrazují na veřejném webu.</p>

            <Field label="Interní poznámky">
              <textarea value={form.internal_notes} onChange={e => update('internal_notes', e.target.value)} rows={6}
                placeholder="Interní poznámky pouze pro štáb..." className={`${inputCls} resize-none`} />
            </Field>

            <Field label="Zodpovědný pracovník">
              <input value={form.staff_assigned} onChange={e => update('staff_assigned', e.target.value)}
        ...
        {/* ── TAB: Historie ── */}
        {tab === 'history' && mode === 'edit' && (
          <div className="bg-white rounded-lg p-5 md:p-6 border border-gray-pale shadow-sm">
            <h2 className="font-display font-extrabold text-xl text-espresso mb-5">
              📋 Historie změn stavů
            </h2>
            ...
            {statusHistory.length === 0 ? (
              <div className="text-center py-10">
                ...
                <p className="text-gray text-sm">Zatím žádné změny stavu</p>
              </div>
            ) : (
              <div className="space-y-3">
                {statusHistory.map((h: any, i: number) => {
                  ...
                  return (
                    <div key={h.id ?? i} className="flex items-start gap-3 p-3 bg-sand/50 rounded-md">
                      <div className="w-2 h-2 rounded-full bg-coral flex-shrink-0 mt-2" />
                      <div className="flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-body text-sm font-semibold text-gray">
                            {labelMap[h.old_status] ?? h.old_status ?? 'Nový záznam'}
                          </span>
                          <span className="text-gray">→</span>
                          <span className="font-body text-sm font-bold text-espresso">
                            {labelMap[h.new_status] ?? h.new_status}
                          </span>
                        </div>
                        {h.note && <p className="text-xs text-brown-mid mt-1">{h.note}</p>}
      ...
      <div className="space-y-5">
        ...
        <div className="bg-white rounded-lg p-5 border border-gray-pale shadow-sm">
          ...
          {/* Poznámka ke změně stavu — zobrazí se jen při editaci */}
          {mode === 'edit' && (
            <div className="mt-3">
              <label className="text-xs font-bold text-brown uppercase tracking-wider block mb-1.5">
                Poznámka ke změně stavu
              </label>
              <input value={statusNote} onChange={e => setStatusNote(e.target.value)}
                placeholder="Volitelná poznámka..." className={inputCls} />
            </div>
          )}
        ...
        {mode === 'edit' && (
          <div className="bg-sand rounded-lg p-5">
            <h3 className="font-display font-bold text-sm text-espresso mb-2 uppercase tracking-wider">
              Karta zvířete
            </h3>
            <p className="text-xs text-gray mb-3">Stáhni kompletní kartu zvířete jako PDF.</p>
            <a
              href={`/admin/animals/${animal?.id}/pdf`}
              target="_blank"
        ...
        {success && (
          <div className="bg-success-bg text-success text-sm font-semibold px-4 py-3 rounded-sm">✓ Uloženo!</div>
        )}

        <Button variant="primary" className="w-full justify-center" loading={loading} onClick={handleSubmit}>
          {mode === 'create' ? '✓ Přidat záznam' : '✓ Uložit změny'}
        </Button>
