        ...
        {results && totalCount === 0 && (
          <div className="text-center py-12">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <Link href="/rescue" className="px-5 py-2.5 rounded-xl font-bold text-sm text-white no-underline bg-rescue">
                Záchranné stanice
              </Link>
            </div>
          </div>
            ...
            {results.rescueCases.length > 0 && (
              <section>
                <div className="flex items-center justify-between mb-4">
                  <h2 className="font-bold text-lg text-text-primary flex items-center gap-2">
                    <span className="w-7 h-7 rounded-lg flex items-center justify-center text-sm"
                      style={{ background: 'var(--rescue-tag-bg)' }}>🦉</span>
                    Záchranné případy
                    <span className="text-sm font-medium px-2 py-0.5 rounded-full"
                  ...
                  <Link href={`/rescue`} className="text-xs font-bold no-underline hover:opacity-70"
                    style={{ color: 'var(--rescue)' }}>
                    Záchranné stanice →
                  </Link>
                </div>
                <div className="space-y-2">
                  {results.rescueCases.map((c: SearchRescueCase) => {
                    ...
                    return (
                      <Link key={c.id} href={`/rescue/${c.id}`} className="no-underline group">
                        <div className="flex items-center gap-4 p-3.5 bg-white rounded-xl border border-border hover:border-rescue/40 hover:-translate-y-0.5 transition-all">
                          <div className="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 relative"
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="font-bold text-text-primary truncate">{c.name ?? c.case_number}</div>
                            <div className="text-xs mt-0.5 truncate text-text-muted">
                    ...
                    <Link key={i.id} href={`/institutions/${i.slug}`} className="no-underline group">
                      <div className="flex items-center gap-4 p-3.5 bg-white rounded-xl border border-border hover:border-coral/40 hover:-translate-y-0.5 transition-all">
                        <div className="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 flex items-center justify-center text-2xl"
                          style={{ background: i.type === 'shelter' ? 'var(--coral-tag-bg)' : 'var(--rescue-tag-bg)' }}>
                          {i.logo_url
...
async function search(q: string): Promise<SearchResults> {
  ...
  return {
    ...
    rescueCases:  (rescueCases.data  ?? []) as unknown as SearchRescueCase[],
