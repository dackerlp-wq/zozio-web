...
export default async function RescuePage({ searchParams }: PageProps) {
  ...
  const cases = await getRescaseCases(params)
    ...
    <main className="min-h-screen bg-warm pt-24 pb-20">
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        <div className="text-center mb-10">
          <span className="inline-flex items-center gap-1.5 bg-rescue-bg text-rescue-dark font-body text-xs font-bold px-4 py-1.5 rounded-pill uppercase tracking-wider mb-4">
            🚑 Ohrožená zvířata
          </span>
          ...
          <p className="text-lg text-brown-mid max-w-[520px] mx-auto leading-relaxed">
            ...
            Podpoř jejich záchrannou stanici.
          </p>
        </div>
        ...
        {cases.length === 0 ? (
          <div className="text-center py-20">
            ...
            <h3 className="font-display font-bold text-2xl text-espresso mb-2">
              Žádné záchranné případy nenalezeny
            </h3>
...
async function getRescaseCases(params: {
  species?: string
  status?: string
  city?: string
}) {
  ...
  if (error) {
    console.error('getRescueCases error:', error)
    return []
  }

  let cases = (data as RescueCase[]) ?? []
  if (params.city) {
    cases = cases.filter(c => c.institution?.city === params.city)
  }
