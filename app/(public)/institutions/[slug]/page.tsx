...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${inst.name} | Zozio`,
    description: inst.short_description ?? inst.description?.slice(0, 155) ?? '',
  }
}
    ...
    <main className="min-h-screen bg-warm pt-24 pb-20">
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray mb-8 font-semibold">
          ...
          <span>·</span>
          <span className="text-espresso">{inst.name}</span>
        </nav>

        {/* Hero instituce */}
        <div className={`rounded-lg p-8 mb-10 ${isShelter ? 'bg-shelter-bg' : 'bg-rescue-bg'}`}>
          <div className="flex items-start justify-between gap-8">
            <div className="flex-1">
              ...
              <h1 className="font-display font-extrabold text-5xl text-espresso mb-2">
                {inst.name}
              </h1>

              <p className={`font-display font-bold text-lg mb-4 ${isShelter ? 'text-coral' : 'text-rescue'}`}>
                {isShelter ? 'Zachraňme opuštěná zvířata' : 'Zachraňme ohrožená zvířata'}
              </p>

              {inst.description && (
                <p className="text-base text-brown-mid leading-relaxed max-w-[600px]">
                  {inst.description}
                </p>
              )}
            </div>
            ...
            {/* Kontakt */}
            <div className="bg-white rounded-lg p-5 shadow-sm min-w-[240px]">
              ...
              {inst.phone && (
                <div className="flex items-center gap-2 text-sm text-brown-mid mb-2">
                  <span>📞</span>
                  <a href={`tel:${inst.phone}`} className="hover:text-coral transition-colors">
                    {inst.phone}
                  </a>
                </div>
              )}
              {inst.email && (
                <div className="flex items-center gap-2 text-sm text-brown-mid mb-2">
                  <span>✉️</span>
                  <a href={`mailto:${inst.email}`} className="hover:text-coral transition-colors truncate">
                    {inst.email}
                  </a>
                </div>
              )}
        ...
        {fundraisers.length > 0 && (
          <section className="mb-12">
            <h2 className="font-display font-extrabold text-3xl text-espresso mb-6">
              💛 Aktivní sbírky
            </h2>
        ...
        {!isShelter && rescueCases.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-3 gap-5">
              {rescueCases.map(c => (
                <RescueCard key={c.id} rescueCase={c} />
              ))}
            </div>
          </section>
        )}
        ...
        {/* CTA — registrace */}
        <div className={`mt-16 rounded-lg p-8 text-center ${isShelter ? 'bg-coral' : 'bg-rescue'}`}>
          <h2 className="font-display font-extrabold text-3xl text-white mb-3">
            Jsi z tohoto útulku?
          </h2>
          <p className="text-white/80 mb-6">
            Přidej se k Zozio a spravuj svůj profil, zvířata a adopce online.
          </p>
...
async function getAnimals(institutionId: string): Promise<Animal[]> {
  const supabase = await createClient()
...
async function getRescueCases(institutionId: string): Promise<RescueCase[]> {
  const supabase = await createClient()
  const { data } = await supabase
    .from('rescue_cases')
    .select('*, institution:institutions(id,name,city,type,slug), species:animal_species(id,name_cs,icon)')
...
async function getFundraisers(institutionId: string): Promise<Fundraiser[]> {
  const supabase = await createClient()
  const { data } = await supabase
    .from('fundraisers')
    .select('*')
